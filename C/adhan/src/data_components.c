...
bool is_valid_time(time_components_t time_components) {
    time_components_t invalid_time_t = INVALID_TIME;

    return !(
            time_components.hours == invalid_time_t.hours &&
            time_components.minutes == invalid_time_t.minutes &&
            time_components.seconds == invalid_time_t.seconds
    );
}

inline time_components_t from_double(double value) {
    time_components_t time_components = INVALID_TIME;
    ...
    time_components = (time_components_t){hours, minutes, seconds};
    return time_components;
}
